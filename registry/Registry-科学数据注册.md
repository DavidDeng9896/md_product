...
| --- | --- | --- | --- | --- | --- | --- |
| V1.0 |  |  | 2026.2.12 |  |  |  |
| V1.1 | 数字字段新增单位设置：支持启用单位→选择单位类别→设置默认单位；注册/填写数据时默认单位自动带出且支持修改；导入导出场景：启用单位的数字字段拆分“数值+单位”两列，导入支持逐行不同单位、严格校验，导出带出存储单位；结果表数字字段同步支持单位设置；关联结果导入同步单位规则 | 科研数据填写需要明确的计量单位上下文，避免数值含义歧义 | 2026.9.17 |  |  |  |
...
| 数字 | ![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/NpQlK5jMypKPNqDv/img/129deefb-ab42-4751-bd4b-babb8a5634c6.png)<br>*   格式：可选项，“数字”、“百分比(%)”, 默认显示“数字”<br>    <br>*   小数位：可选项，"整数","不限位数","1位","2位","3位","4位","5位"；<br>    <br>*   默认值：填写数字类型    <br>*   **单位设置（新增）**：<br>    <br>    1.  **启用单位**：开关项，默认关闭；关闭时下方单位设置隐藏；从启用切回关闭时，已选单位类别与默认单位自动清空<br>        <br>    2.  **单位类别**：启用后开放，从系统预设的计量类别中选择：温度、质量、体积、物质的量、物质的量浓度、密度、长度、面积、时间、包装数量、纯度、其他（“其他”类别置灰不可选）；单位类别为系统预设，不通过数据字典管理<br>        <br>    3.  **默认单位**：选择类别后开放，展示该类别下的单位列表供选择（如“质量”→ ng/kg/g/mg/μg）；未选类别时置灰不可操作；切换类别后已选默认单位自动清空<br>        <br>    4.  **填写带出**：注册对象、填写数据时，默认单位作为输入框后缀自动带出显示；**用户填写时可修改单位**，修改仅影响当前数据行，不改变字段配置的默认单位<br>        <br>    5.  单位修改的范围为当前类别下的单位列表，不支持跨类别修改<br>        <br>    6.  **保存校验**：启用单位但未设置默认单位时允许保存（字段设置摘要显示“未设置”），填写数据时单位位置显示为空，由用户手动选择；导入时同理：弹窗表格单位下拉显示“请选择单位”、Excel模板单位列留空，均由用户逐行填写，数值非空时单位必选<br>    <br>数字校验显？？ |
...
| **B、功能点** |  |
| **功能点** | **描述** |
| 手动注册 | ![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/NpQlK5jMypKPNqDv/img/ef3b6032-8bb6-46b4-8bd1-9127887f154e.png)<br>点击列表“新增”，显示注册弹窗<br>1.  基础信息部分<br>    <br>    1.  显示当前注册的对象名称及所属类型(如上图所示)<br>        <br>    2.  名称，必填，是否预先生成名称和是否能够填写，取决于对象配置<br>        <br>    3.  所属部门，必填，可选择多个部门<br>        <br>    4.  别名：可输入多个别名(如图)<br>        <br>2.  自定义信息<br>    <br>    1.  只需展示和填写，需要用户输入的内容，不显示关联查找、公式、汇总类型的字段；数字类型字段若在字段设置中启用了单位，填写时默认单位作为输入框后缀自动带出，用户可在当前单位类别的单位列表中修改单位（修改仅影响本条数据，不改变字段配置的默认单位）<br>        <br>3.  针对DNA、RNA、氨基酸序列类型的对象，用户可上传序列内容（见下文）<br>    <br>4.  可见范围：<br>    <br>    1.  项目成员可见，控制新增的实体项目下的成员是否可见，默认不勾选<br>        <br>    2.  所属部门成员可见，控制新增的实体部门下的成员是否可见，默认不勾选 |
...
| 上传本地文件进行 | ![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/NpQlK5jMypKPNqDv/img/17678808-c240-4257-8623-9dbc56e5bf7b.png)<br>支持导出列表数据表模板<br>填写数据内容后，导入数据内容，在列表中显示；完成数据的预录入<br>    <br>**数字字段单位列规则（新增）**：<br>1.  启用单位的数字字段在模板中拆分为两列：“字段名（数值）”+“字段名（单位）”，支持每行填写不同单位；<br>2.  单位列校验：必须是字段配置类别下的合法单位，非法值阻断导入；<br>3.  单位列默认值：已设默认单位则预填默认单位（可修改）；未设默认单位则留空，由用户填写；<br>4.  校验规则：数值非空而单位为空 → 阻断导入；单位已填而数值为空 → 允许（该字段整行为空，单位忽略）；<br>5.  落库口径：数值与所选单位原样存储，不做单位换算；<br>6.  未启用单位的数字字段，模板保持单列不变 |
